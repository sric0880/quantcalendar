...
- 支持集合竞价时间查询
- 支持查询不同周期的K线时间。支持左侧和右侧K线时间（数字货币是左侧K线时间，期货股票是右侧K线时间）。
...
```sh
git archive --format=tar.gz --output /nas/public/qiong/third_parties/quantcalendar-2025.09.tar.gz no-depends
```
