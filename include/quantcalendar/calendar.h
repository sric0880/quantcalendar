...
public:
  ...
  tradedays_iterator TradedaysUpper(sec_t dt) const { return data_.TradedaysUpper(dt); }
  tradedays_iterator TradedaysLower(sec_t dt) const { return data_.TradedaysLower(dt); }
  CalendarData::iter_range<tradedays_iterator> TradedaysBetween(sec_t start, sec_t end) const { return data_.TradedaysBetween(start, end); }
  month_end_iterator MonthEndUpper(sec_t dt) const { return data_.MonthEndUpper(dt); }
  month_end_iterator MonthEndLower(sec_t dt) const { return data_.MonthEndLower(dt); }
  CalendarData::iter_range<month_end_iterator> MonthEndBetween(sec_t start, sec_t end) const { return data_.MonthEndBetween(start, end); }
  month_begin_iterator MonthBeginUpper(sec_t dt) const { return data_.MonthBeginUpper(dt); }
  month_begin_iterator MonthBeginLower(sec_t dt) const { return data_.MonthBeginLower(dt); }
  CalendarData::iter_range<month_begin_iterator> MonthBeginBetween(sec_t start, sec_t end) const { return data_.MonthBeginBetween(start, end); }
  week_end_iterator WeekEndUpper(sec_t dt) const { return data_.WeekEndUpper(dt); }
  week_end_iterator WeekEndLower(sec_t dt) const { return data_.WeekEndLower(dt); }
  CalendarData::iter_range<week_end_iterator> WeekEndBetween(sec_t start, sec_t end) const { return data_.WeekEndBetween(start, end); }
  week_begin_iterator WeekBeginUpper(sec_t dt) const { return data_.WeekBeginUpper(dt); }
  week_begin_iterator WeekBeginLower(sec_t dt) const { return data_.WeekBeginLower(dt); }
  CalendarData::iter_range<week_begin_iterator> WeekBeginBetween(sec_t start, sec_t end) const { return data_.WeekBeginBetween(start, end); }
  weekday_iterator WeekDayUpper(sec_t dt, int weekday) const { return data_.WeekDayUpper(dt, weekday); }
  weekday_iterator WeekDayLower(sec_t dt, int weekday) const { return data_.WeekDayLower(dt, weekday); }
  CalendarData::iter_range<weekday_iterator> WeekDayBetween(sec_t start, sec_t end, int weekday) const { return data_.WeekDayBetween(start, end, weekday); }
  void InitSpecialSessions(ankerl::unordered_dense::map<sec_t, SpecialSessions> &&sessions);
  /**
...
private:
  ...
  const SpecialSessions *GetSpecialSessions(sec_t dt) const;
